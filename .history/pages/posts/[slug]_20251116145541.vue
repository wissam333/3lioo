...
// AUTHOR (dependent)
const { data: authorData } = await useFetch(
  () => (post.value ? `/api/authors/${post.value.author}` : null),
  {
    key: `author-${slug}`,
    default: () => null,
  }
...
// RELATED POSTS (dependent)
const { data: relatedPosts } = await useFetch(
  () => (post.value ? `/api/posts?category=${post.value.category}` : null),
  {
    key: `related-${slug}`,
    transform: (posts) =>
      posts?.filter((p) => p.slug !== post.value.slug).slice(0, 3) || [],
  }
);

// MOST WATCHED
const { data: mostWatched } = await useFetch("/api/posts/most-watched", {
  key: `most-watched`,
});
...
// post is your fetched article (post.body or post.content)
const renderedMarkdown = computed(() => {
  if (!post.value) return "";
  // note: some of your API responses might use `body` or `content`
...
// Get short threat level labels for compact display
...
// Simulate trend data (you can replace with actual data)
