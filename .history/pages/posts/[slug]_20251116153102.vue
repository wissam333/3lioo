...
import { documentToString } from '@nuxt/content'

const { data: postBody } = await useAsyncData(`post-${slug}`, () =>
  queryContent("posts")
    .where({ _path: `/posts/${slug}` })
    .findOne()
);
...
const renderedMarkdown = computed(() => {
  ...
  const raw = documentToString(postBody.value);
...
//   const raw = postBody.value.body ?? postBody.value.content ?? "";
//   return md.render(raw);
// });
