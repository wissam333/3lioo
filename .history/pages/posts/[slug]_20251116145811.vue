...
// AUTHOR (only if you have authors folder)
const { data: authorData } = await useAsyncData(`author-${slug}`, () =>
  post.value
    ? queryContent("authors").where({ name: post.value.author }).findOne()
    : null
);
...
// MOST WATCHED (visits must come from frontmatter)
const { data: mostWatched } = await useAsyncData(`most-watched`, () =>
  queryContent("posts").sort({ visits: -1 }).limit(4).find()
);
