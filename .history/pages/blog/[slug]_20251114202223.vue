  ...
  <div v-if="post" class="article-page">
    ...
    <main class="content">
      ...
      <article class="article-body">
        <div ref="content" v-html="$md.render(post.body)"></div>
      </article>
...
const fetchAuthorData = async (authorName) => {
  try {
    const { data } = await useAsyncData(`author-${authorName}`, () =>
      queryContent("authors").where({ name: authorName }).findOne()
    );
...
const fetchRelatedPosts = async () => {
  ...
  try {
    const { data } = await useAsyncData(`related-${slug}`, () =>
      queryContent("blog")
        .where({
...
const fetchMostWatched = async () => {
  try {
    const { data } = await useAsyncData("most-watched", () =>
      queryContent("blog").sort({ visits: -1 }).limit(5).find()
    );
    mostWatched.value = data.value || [];
  } catch (error) {
    console.error("Failed to fetch most watched posts", error);
  }
};
...
const increaseVisit = async () => {
  // This would typically be an API call to update the visit count
  console.log("Visit recorded for:", slug);
};
...
useSeoMeta({
  title: () =>
    post.value?.seo?.metaTitle || post.value?.title || "Security Article",
  description: () =>
    post.value?.seo?.metaDescription ||
    post.value?.excerpt ||
    "Read this security analysis article",
  ogTitle: () =>
    post.value?.seo?.metaTitle || post.value?.title || "Security Article",
  ogDescription: () =>
    post.value?.seo?.metaDescription ||
  ...
  robots: () =>
    post.value?.seo?.noIndex ? "noindex, nofollow" : "index, follow",
});
  ...
  script: [
    {
      ...
      innerHTML: computed(() => {
        ...
        return {
          ...
          image: post.value.featuredImage?.url
