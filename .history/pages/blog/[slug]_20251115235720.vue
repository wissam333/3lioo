...
const fetchPost = async () => {
  const { getPostBySlug } = usePostBySlug();
...
const fetchAuthorData = async (authorName) => {
  const { getAuthorByName } = useAuthorByName();
  authorData.value = await getAuthorByName(authorName);
};
...
// Fetch most watched posts (top by visits, exclude current)
const fetchMostWatched = async () => {
  const { getAllPosts } = useAllPosts();
  const posts = await getAllPosts();

  mostWatched.value = posts
