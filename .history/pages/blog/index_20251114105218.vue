    ...
    <section class="blog-filters">
      <div class="container">
        <div class="filters-grid">
          ...
          <div class="filter-group">
            <select v-model="selectedThreatLevel" class="filter-select">
        ...
        <div class="active-filters" v-if="hasActiveFilters">
          ...
          <span v-if="selectedThreatLevel" class="filter-tag">
            ...
            <button @click="selectedThreatLevel = ''" class="filter-remove">
              ×
            </button>
          </span>
...
const { data: allPosts } = await useAsyncData("blog-posts", () => {
  return queryContent("/content/posts").sort({ date: -1 }).find();
});
...
const categories = computed(() => {
  ...
  return [
    ...new Set(allPosts.value.map((post) => post.category || "Security")),
  ];
});
...
const filteredPosts = computed(() => {
  ...
  if (selectedCategory.value) {
    filtered = filtered.filter(
      (post) => (post.category || "Security") === selectedCategory.value
    );
  }
  ...
  if (selectedThreatLevel.value) {
    filtered = filtered.filter(
      (post) => post.threatLevel === selectedThreatLevel.value
    );
  }
